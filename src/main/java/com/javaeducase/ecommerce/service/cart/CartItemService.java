...
public class CartItemService {
    ...
    public CartItem createCartItem(Long offerId, int quantity) {
        ...
        if (quantity <= 0) {
            throw new IllegalArgumentException(" оличество должно быть больше 0");
        }

        Offer offer = offerRepository.findById(offerId)
                .orElseThrow(() -> new OfferNotFoundException("ѕредложение с id: " + offerId + " не найдено"));

        if (offer.getIsDeleted()) throw new OfferIsDeletedException("ѕредложение было удалено");
        if (!offer.getIsAvailable()) throw new OfferIsUnavailableException("ѕредложение на данный момент недоступно");
        if (quantity > offer.getStockQuantity()) {
            throw new IllegalArgumentException(" оличество не может быть больше, чем доступно на складе");
        }
    ...
    public CartItem updateCartItem(Long cartItemId, int newQuantity) {
        CartItem cartItem = cartItemRepository.findById(cartItemId)
                .orElseThrow(() -> new CartItemNotFoundException("Ёлемент корзины с id: " + cartItemId + " не найден"));

        Offer offer = cartItem.getOffer();

        if (newQuantity < 0) {
            throw new IllegalArgumentException(" оличество не может быть отрицательным");
